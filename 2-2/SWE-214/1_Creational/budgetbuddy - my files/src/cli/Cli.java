    ...
    private void processCommand(String line) {
        ...
        try {
            switch (command) {
                ...
                case "export":
                    if (parts.length < 3) {
                        System.out.println("Usage: export txt <outpath> or export html <outpath>");
                    } else {
                    // parts[1] is the type (txt/html), parts[2] is the path
                    commandHandler.handleExport(parts[1], parts[2]); 
                    }
                    break;
