...
import io.CsvLoader;
import io.ReportBuilder;
import io.ReportDirector;
...
public class CommandHandler {
    ...
    public void handleLoad(String filePath) {
        try {
            ...
            List<Expense> expenses = CsvLoader.loadFromFile(filePath);
    ...
    // public void handleExportTxt(String outputPath) {
    //     try {
    //         TxtReportBuilder builder = new TxtReportBuilder(outputPath);
    //         ReportDirector director = new ReportDirector(builder);
    //         director.constructReport(mainRepository);
    ...
    // public void handleExportHtml(String outputPath) {
    //     try {
    //         HtmlReportBuilder builder = new HtmlReportBuilder(outputPath);
    //         ReportDirector director = new ReportDirector(builder);
    //         director.constructReport(mainRepository);
     ...
     * Uses Factory Pattern to create the builder, and Builder Pattern to generate the report.
     */
    public void handleExport(String type, String outputPath) {
        try {
            // 1. FACTORY PATTERN: Get the correct builder without knowing the class name
            ReportBuilder builder = ReportFactory.createBuilder(type, outputPath);

            // 2. BUILDER PATTERN: Pass the builder to the director
            ReportDirector director = new ReportDirector(builder);
            director.constructReport(mainRepository);
            
            System.out.println(type.toUpperCase() + " report written to: " + outputPath);
        } catch (IOException e) {
            System.err.println("Error writing report: " + e.getMessage());
        } catch (IllegalArgumentException e) {
            System.err.println("Error: " + e.getMessage());
