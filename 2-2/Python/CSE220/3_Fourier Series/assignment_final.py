...
class FourierSeries:
    ...
    def approximate(self, x):
        ...
        approximation = np.full_like(x, a0) # Start with a0 (Average/DC)

        for n in range(1, self.terms + 1):
            an = self.calculate_an(n)
            bn = self.calculate_bn(n)
            
            approximation += (an * np.cos(n * np.pi * x / self.L) + 
